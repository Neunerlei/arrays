...
class ArrayPaths {
	...
	public function parsePath($path, string $separator = "."): array {
		...
		if (!is_string($path) && !is_numeric($path) && !is_array($path))
			throw new InvalidArgumentException("The given path: " . json_encode($path) . " is not valid! Only strings, numbers and arrays are supported!");
		
		// Check if the given path array is valid
		if (is_array($path)) {
			$parts = array_values($path);
			array_walk_recursive($path, function ($v) use ($parts) {
				if (!is_string($v) && !is_numeric($v) && !is_array($v))
					throw new InvalidArgumentException(
						"The given path array: " . json_encode($parts) . " should only contain numbers, strings and arrays!");
			});
		} else {
			$path = (string)$path;
			$cacheKey = $path . $separator;
			if (!empty($this->pathCache[$cacheKey])) return $this->pathCache[$cacheKey];
			...
			if (!$this->canUseFastLane($path, $separator)) {
				...
				$parts = array_values(array_filter(
					array_map("trim", explode($separator, $braces[0])), function ($v) {
					return $v !== "" && $v !== NULL;
				}));
				...
				if (count($braces) > 1) {
					...
					$parts = json_decode($partsString, TRUE);
				}
	...
	protected function hasWalker(array $list, array $path): void {
		...
		if (empty($list) || empty($keys)) throw new Exception();
		foreach ($keys as $key) {
			// Handle nested paths
			if (is_array($key)) {
			...
			if (!array_key_exists($key, $list)) throw new Exception();
			if (!$isLastKey) {
				if (!is_array($list[$key])) throw new Exception();
	...
	protected function getWalker(array $list, array $path, $default, bool $isNested = FALSE) {
		...
		else if ($key === static::KEY_TYPE_WILDCARD && reset($keys) === 0)
	...
	protected function removeWalker(array &$list, array $path, bool $keepEmpty): void {
		...
		foreach ($keys as $key) {
			...
			if (is_array($list[$key])) $this->removeWalker($list[$key], $path, $keepEmpty);
			if (!$keepEmpty && empty($list[$key])) unset($list[$key]);
		}
	}
	 ...
	 * @see filter()
	 */
	protected function filterWalker(array &$list, array $path, array $localPath,
									callable $callback, string $separator, array $inputArray): void {
		[$keys, $isLastKey] = $this->initWalkerStep($list, $path);
		foreach ($keys as $key) {
			$localPath[] = $key;
			if (is_array($key)) $this->filterWalker($list, $key, $localPath, $callback, $separator, $inputArray);
			else {
				if ($isLastKey) $list[$key] = call_user_func($callback, $list[$key], $key, $localPath, $inputArray);
				else if (is_array($list[$key])) $this->filterWalker($list[$key], $path, $localPath, $callback, $separator, $inputArray);
			}
			array_pop($localPath);
		}
	}
	 ...
	 */
	public function getList(array $input, array $valueKeys, string $keyKey = "",
							$default = NULL, string $separator = "."): ?array {
		...
		foreach ($input as $row) {
			...
			// Append to result
			if ($hasKeyKey && !empty($keyKeyValue)) $result[$keyKeyValue] = $rowValues;
			else $result[] = $rowValues;
		}
	...
	protected function resolveBracesInPath(string $path, string $separator): array {
		...
		for ($i = 0; $i < $length; $i++) {
			...
			if ($char === "[") {
				...
				if (!isset($braces[$braceId])) $braces[$braceId] = "";
			}
			$braces[$braceId] .= $char;
			if ($char === "]") $braceId = (int)array_pop($bracesPath);
		}
		
		// Split the stored braces into parts
		foreach ($braces as $braceId => $brace) {
			if ($braceId === 0) continue;
			
			// Split at the comma
			$brace = trim($brace, "[],");
			preg_match_all("/(\\.|[^,])+/si", $brace, $braceParts);
			$braces[$braceId] = json_encode(
				array_filter(array_map(function ($part) use ($separator) {
					$part = trim($part);
					if (empty($part)) return NULL;
					if (stripos($part, $separator)) return $this->parsePath($part, $separator);
					return $part;
				}, $braceParts[0]))
			);
		}
	...
	protected function canUseFastLane($path, string $separator): bool {
		...
		if (stripos($path, $separator) !== FALSE) return FALSE;
		foreach (static::ESCAPABLE_CHARS as $escapable)
			if (stripos($path, $escapable) !== FALSE) return FALSE;
		return TRUE;
	}
